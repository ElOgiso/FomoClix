    ...
    try {
      ...
      if (ethereum) {
        try {
          ...
          console.warn('[ConnectWalletStep Web3 Error]', web3Err);
          if (web3Err?.message?.includes('4001') || web3Err?.message?.toLowerCase().includes('rejected') || web3Err?.message?.toLowerCase().includes('denied')) {
            throw new Error('Signature rejected. Please approve the sign request in your wallet to continue.');
          }
        }
      }

      // If no Web3 extension or signature was cancelled, offer Instant Demo Wallet option
      if (!address || !signature) {
        address = '0xFA887A39E22555e1a3f815A9aC730366ec0e05E4';
        const challenge = await authApi.getChallenge(address);
        message = challenge.message;
        signature = '0x_simulated_sig_' + Math.random().toString(36).substring(7);
      }
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      try {
        ...
        // Fallback when browser blocks popups in iframe
        const userEmail = prompt('Google login popup was restricted by browser/iframe. Enter your Gmail address to sign in:', 'mauriceogiso@gmail.com');
        if (!userEmail) {
          throw new Error('Google sign-in cancelled.');
        }
        idToken = 'direct_gmail_' + btoa(userEmail);
        userObj = { email: userEmail, uid: 'gmail_' + btoa(userEmail) };
      }
      ...
      onConnected(
        result.jwt_token, 
        result.profile.email || userObj?.email || result.profile.uid, 
