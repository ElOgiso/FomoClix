...
export default function App() {
  const [authState, setAuthState] = useState<AuthState>('AUTH_RESTORING');
  ...
  /**
   * LandingScreen passes (jwt, addressOrUid, role) and optionally the full login payload.
   * Prefer the login response for session state so a flaky /auth/session does not log the user out.
   */
  const handleLoginSuccess = async (
    jwtToken: string,
    addressOrUid: string,
    roleFromLogin: string = 'user',
    loginPayload?: {
    ...
    localStorage.setItem('jwt_token', jwtToken);
    if (addressOrUid) {
      localStorage.setItem('fomoclix_wallet_address', addressOrUid);
    }
    setToken(jwtToken);
    setAuthState('AUTH_RESTORING');
    
    if (jwtToken === 'demo_token' || jwtToken.includes('fomoclix_demo_')) {
      setMockMode(true);
      ...
      setFullWalletAddress('0xFA887A39E22555e1a3f815A9aC730366ec0e05E4');
      setWalletAddress('0xFA88...05E4');
      setAuthState('AUTHENTICATED');
      return;
    }
    ...
    const applySession = (session: {
      ...
      setUserRole(session.profile.role || roleFromLogin || 'user');
      setOnboardingComplete(!!session.onboardingComplete);
      setUserProfile({
        uid: session.profile.uid,
        email: session.profile.email,
        role: session.profile.role || roleFromLogin || 'user',
        primaryAddress: session.profile.primaryAddress || addressOrUid || '',
        plan: session.plan,
    ...
    // Prefer /auth/session when available; fall back to login payload (never silent logout)
    try {
      const session = await authApi.restoreSession();
      applySession(session);
    } catch (err) {
      console.warn('[Login Success] restoreSession failed; using login payload fallback.', err);
      if (loginPayload?.profile) {
        applySession({
          profile: loginPayload.profile,
          subscription: loginPayload.subscription || { active: false, plan: null },
          onboardingComplete: !!loginPayload.onboardingComplete,
          tradingWallet: loginPayload.tradingWallet || null,
          plan: loginPayload.plan || null,
        });
        return;
      }
      // Minimal session from args so user still enters onboarding / dashboard path
      applySession({
        profile: {
          uid: addressOrUid || 'unknown',
          email: '',
          role: roleFromLogin || 'user',
          primaryAddress: addressOrUid || '',
        },
...
- Recommended Priority Fee: 0.05 GWEI.`;
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      // 0. Check if we have a demo token saved
      const localToken = localStorage.getItem('jwt_token');
      if (localToken === 'demo_token' || (localToken && localToken.includes('fomoclix_demo_'))) {
        if (active) {
          setMockMode(true);
          setMockModeState(true);
          setToken(localToken);
        ...
        try {
          ...
          if (result.jwt_token && active) {
            ...
            setUserProfile({
              ...
              plan: result.plan,
              subscriptionActive: result.subscription.active,
        ...
        try {
          ...
          if (active) {
            ...
            setUserProfile({
              ...
              primaryAddress: session.profile.primaryAddress,
              plan: session.plan,
              subscriptionActive: session.subscription.active,
              onboardingComplete: session.onboardingComplete
            });
      ...
      // 3. No valid authentication resolved
      if (active) {
        handleLogout();
      }
    };
  ...
  if (authState === 'UNAUTHENTICATED') {
    return <LandingScreen onNext={() => {}} onLoginSuccess={handleLoginSuccess} />;
  }

  if (!token || !onboardingComplete) {
    return <OnboardingFlow token={token} onComplete={handleOnboardingComplete} onLogout={handleLogout} />;
  }
