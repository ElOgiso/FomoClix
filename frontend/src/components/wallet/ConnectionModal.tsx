...
export const ConnectionModal: React.FC<ConnectionModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleConnectAndSign = async () => {
    setConnecting(true);
    try {
      const mockAddress = selectedChain === 'ethereum' 
      ...
      const challengeResult = await authApi.getChallenge(mockAddress);
      const challengeMsg = challengeResult.message;

      console.log('[ConnectionModal] Received auth challenge message:', challengeMsg);

      // 2. Sign message using wallet client (simulated signature in this demo shell)
      const simulatedSignature = '0x_simulated_sig_' + Math.random().toString(36).substring(7);

      // 3. Verify signature via AuthProvider to get JWT token
      const success = await login(mockAddress, challengeMsg, simulatedSignature);
      if (success) {
        onSuccess();
        onClose();
      }
