...
// gateway.js
var ZoraBotContainer = class extends Container {
  static {
    __name(this, "ZoraBotContainer");
  }
  ...
  static outboundByHost = {
    "d1.local": /* @__PURE__ */ __name(async (request, env2, ctx) => {
      try {
        const body = await request.json();
        const { sql, params, method } = body;
        let result;
        if (method === "run") {
          result = await env2.DB.prepare(sql).bind(...params || []).run();
        } else if (method === "all") {
          result = await env2.DB.prepare(sql).bind(...params || []).all();
        } else if (method === "exec") {
          result = await env2.DB.exec(sql);
        } else if (method === "batch") {
          const statements = sql.map((s, idx) => env2.DB.prepare(s).bind(...params[idx] || []));
          result = await env2.DB.batch(statements);
        } else {
          result = await env2.DB.prepare(sql).bind(...params || []).all();
        }
...
var gateway_default = {
  async fetch(request, env2) {
    const id = env2.ZORA_BOT.idFromName("global-bot-instance");
    const container = env2.ZORA_BOT.get(id);
